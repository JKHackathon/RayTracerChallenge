...
Vector Cone::local_normal_at(const Point local_p) const {
    ...
    // EPSILON necessary to prevent issues w/ point not being exactly on cap
    // TODO: is dist less than 1 or less than current radius (max/min)?
    if (dist < maximum && local_p.y >= maximum - EPSILON) {
        return Vector(0, 1, 0);
    }
    else if (dist < minimum && local_p.y <= minimum + EPSILON) {
        return Vector(0, -1, 0);
    }
...
IntersectionRecord Cone::local_intersect(const Ray local_r) const {
    ...
    return xs;
    // TODO: Why am i allowed to return
    // `Intersection(t0,this), Intersection(t1,this)`
    // and only one intersection is added to the pack
}

bool Cone::check_cap(Ray r, double t, double plane_y) const {
    auto x = r.origin.x + t * r.dir.x;
    auto z = r.origin.z + t * r.dir.z;
    return (pow(x, 2) + pow(z, 2)) <= abs(plane_y);
}
