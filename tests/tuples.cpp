...
TEST_CASE("A tuple with w=1.0 is a point", "[tuple]") {
    ...
    REQUIRE(double_equal(p.x, 4.3));
    REQUIRE(double_equal(p.y, -4.2));
    REQUIRE(double_equal(p.z, 3.1));
    REQUIRE(double_equal(p.w, 1.0));
...
TEST_CASE("A tuple with w=0 is a vector", "[tuple]") {
    ...
    REQUIRE(double_equal(v.x, 4.3));
    REQUIRE(double_equal(v.y, -4.2));
    REQUIRE(double_equal(v.z, 3.1));
    REQUIRE(double_equal(v.w, 0.0));
...
TEST_CASE("Computing the magnitude of vector(1, 2, 3)", "[tuple]") {
    Vector v(1, 2, 3);
    REQUIRE(double_equal(v.magnitude(), sqrt(14)));
}
...
TEST_CASE("Normalizing vector(1, 2, 3)", "[tuple]") {
    ...
    REQUIRE(double_equal(v.normalized().magnitude(), 1));
}

TEST_CASE("Dot product", "[tuple]") {
    Vector a(1, 2, 3);
    Vector b(2, 3, 4);
    REQUIRE(double_equal(a.dot(b), 20));
}
