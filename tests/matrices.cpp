...
TEST_CASE("Constructing and inspecting a 4x4 matrix", "[matrices]") {
    ...
    REQUIRE(double_equal(m(0, 0), 1));
    REQUIRE(double_equal(m(0, 3), 4));
    REQUIRE(double_equal(m(1, 0), 5.5));
    REQUIRE(double_equal(m(1, 2), 7.5));
    REQUIRE(double_equal(m(2, 2), 11));
    REQUIRE(double_equal(m(3, 0), 13.5));
    REQUIRE(double_equal(m(3, 2), 15.5));
}
...
TEST_CASE("A 2x2 matrix ought to be representable", "[matrices]") {
    ...
    REQUIRE(double_equal(m(0, 0), -3));
    REQUIRE(double_equal(m(0, 1), 5));
    REQUIRE(double_equal(m(1, 0), 1));
    REQUIRE(double_equal(m(1, 1), -2));
}
...
TEST_CASE("A 3x3 matrix ought to be representable", "[matrices]") {
    ...
    REQUIRE(double_equal(m(0, 0), -3));
    REQUIRE(double_equal(m(1, 1), -2));
    REQUIRE(double_equal(m(2, 2), 1));
}

TEST_CASE("Matrix equality with identical matrices", "[matrices]") {
    Matrix A(4, 4);
    Matrix B(4, 4);
    std::initializer_list<double> values = { 1, 2, 3, 4, 5, 6, 7, 8,
                                           9, 8, 7, 6, 5, 4, 3, 2 };
    A = values;
    B = values;
...
TEST_CASE("Calculating the inverse of a matrix", "[matrices]") {
    ...
    REQUIRE(double_equal(B(3, 2), -160.0f / 532));
    REQUIRE(A.cofactor(3, 2) == 105);
    REQUIRE(double_equal(B(2, 3), 105.0f / 532));
    REQUIRE(B == A_inverse);
}
